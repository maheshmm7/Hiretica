...
class AIPipeline:
    ...
    def run_workspace(self, job_id: str, job_description: str) -> dict:
        ...
        avg_score = sum(c["final_score"] for c in workspace_candidates) / total if total else 0
        avg_hybrid = sum(c["hybrid_score"] for c in workspace_candidates) / total if total else 0
        avg_recruiter = sum(c["recruiter_score"] for c in workspace_candidates) / total if total else 0
        avg_behavior = sum(c["behavior_score"] for c in workspace_candidates) / total if total else 0
        
        # Generate Charts (Buckets)
        recruiter_dist = {"Low": 0, "Average": 0, "Strong": 0, "Exceptional": 0}
        for c in workspace_candidates:
            s = c["recruiter_score"]
            if s < 60: recruiter_dist["Low"] += 1
            elif s < 75: recruiter_dist["Average"] += 1
            elif s < 90: recruiter_dist["Strong"] += 1
            else: recruiter_dist["Exceptional"] += 1
            
        behavior_dist = {"High Risk": 0, "Moderate Risk": 0, "Low Risk": 0, "Safe": 0}
        for c in workspace_candidates:
            s = c["behavior_score"]
            if s < 50: behavior_dist["High Risk"] += 1
            elif s < 70: behavior_dist["Moderate Risk"] += 1
            elif s < 85: behavior_dist["Low Risk"] += 1
            else: behavior_dist["Safe"] += 1
