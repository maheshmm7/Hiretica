...
def test_evidence_formatter():
    ...
    evidence = [
        BehaviorEvidence(
            module="profile_quality", reason="low market validation", impact=-0.1
        ),
        BehaviorEvidence(
            module="availability", reason="immediate notice period", impact=0.2
        ),
    ]
...
def test_reason_builder():
    ...
    reason, t_id = builder.build_reason(
        0.9, 0.9, ["immediate availability"], ["extended notice period"]
    )
