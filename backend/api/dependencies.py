...
def ensure_cache_exists(cache_dir: str):
    ...
    try:
        ...
        raise RuntimeError(f"Deployment Error: Failed to download cache archive from {archive_url}: {e}")
    ...
    try:
        ...
        raise RuntimeError(f"Deployment Error: Failed to extract cache archive. Ensure it is a valid ZIP file: {e}")
    finally:
        if os.path.exists(zip_path):
            os.remove(zip_path)
            
    if not verify_cache_artifacts(cache_dir):
        raise RuntimeError("Deployment Error: Cache archive was extracted but required files are still missing. Verify the ZIP structure.")
